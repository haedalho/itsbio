...
export default async function CrisprKoCellLinesPage({ searchParams }: { searchParams?: Promise<{ q?: string; page?: string }> | { q?: string; page?: string } }) {
  ...
  const records = all.filter((record) => abmRecordBelongsToProductPath(record, ["cellular-materials", "cell-library-collections", "crispr-ko-cell-lines"], "CRISPR KO Cell Lines"));
  ...
  return <CellLibraryShell title="CRISPR KO Cell Lines" active="crispr-ko-cell-lines">
    ...
    <AbmStagedCatalog kind="product" records={records} query={query} page={page} basePath="/products/abm/cellular-materials/cell-library-collections/crispr-ko-cell-lines" />
  </CellLibraryShell>;
}
