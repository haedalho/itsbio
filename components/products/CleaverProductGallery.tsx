  ...
  return (
    <div>
      <div className="relative aspect-square overflow-hidden border border-slate-200 bg-white">
        ...
        />
        {images.length > 1 ? <div className="absolute right-4 top-4 bg-slate-950/75 px-3 py-1.5 text-xs font-medium text-white backdrop-blur-sm">{selected + 1} / {images.length}</div> : null}
      ...
      {zoomed ? (
        <div role="dialog" aria-modal="true" aria-label={`${title} full-resolution product photograph`} className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/88 p-4 md:p-10" onClick={() => setZoomed(false)}>
          ...
          <div className="relative h-[min(90vh,1200px)] w-[min(94vw,1600px)]" onClick={(event) => event.stopPropagation()}><Image src={active} alt={title} fill unoptimized={directManufacturer} quality={90} sizes="94vw" className="object-contain" /></div>
        </div>
      ) : null}
    </div>
  );
