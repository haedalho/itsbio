...
if (!detail.includes("  verifiedGallery,")) {
  detail = replaceRequired(detail, "  images,\n  kentSections,", "  images,\n  verifiedGallery,\n  kentSections,", "detail destructure");
}
if (!detail.includes("  verifiedGallery?: boolean;")) {
  detail = replaceRequired(detail, "  images: Img[];\n  kentSections?: KentSection[];", "  images: Img[];\n  verifiedGallery?: boolean;\n  kentSections?: KentSection[];", "detail prop type");
}
if (!detail.includes("            verifiedGallery={verifiedGallery}")) {
  detail = replaceRequired(detail, "            images={galleryImages}\n            title={title}", "            images={galleryImages}\n            title={title}\n            verifiedGallery={verifiedGallery}", "gallery prop");
}
...
if (!route.includes("    kentOfficialGalleryStatus,")) {
  route = replaceRequired(
    ...
    "official gallery query fields",
  );
}

if (!route.includes("  const stagedOfficialImages =")) {
  const before = `  const productImages = normalizeImages(product, title);
  ...
  const after = `  const stagedOfficialImages = ["STAGING", "APPROVED"].includes(String(product?.kentOfficialGalleryStatus || ""))
  ...
  route = replaceRequired(route, before, after, "route gallery selection");
}
...
if (!route.includes("          verifiedGallery={verifiedGallery}")) {
  route = replaceRequired(
    route,
    "          images={images}\n          kentSections={kentSections as any[]}",
    "          images={images}\n          verifiedGallery={verifiedGallery}\n          kentSections={kentSections as any[]}",
    "route verified gallery prop",
  );
}
fs.writeFileSync(routeFile, route, "utf8");

console.log("Applied Kent official gallery data architecture v2.");
