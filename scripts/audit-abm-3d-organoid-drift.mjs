  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        ...
        "user-agent": "Mozilla/5.0 (compatible; ITSBIO-ABM-DriftAudit/1.0)",
      },
    });
...
const docs = await client.fetch(`*[
  ...
  && version == $version
]{_id,kind,records[]{pathKey,title,sourceUrl,collectedAt,html,images}}`, { version: VERSION });
...
for (const doc of docs || []) {
  for (const record of doc.records || []) {
    ...
    candidates.push({
      ...
      storedText: textFromHtml(record.html),
    });
  }
}

console.log("===== SANITY CANDIDATES =====");
console.log(JSON.stringify(candidates.map((row) => ({
  ...
  htmlSha256: digest(row.html),
  textSha256: digest(row.storedText),
...
for (const url of OFFICIAL_URLS) {
  try {
    ...
    console.log(JSON.stringify({ requestedUrl: url, error: String(error?.stack || error) }, null, 2));
  }
}
...
for (const row of candidates) {
  ...
  for (const url of routeCandidates) {
    try {
      ...
      console.log(JSON.stringify({ pathKey: row.pathKey, url, error: String(error?.stack || error) }, null, 2));
    }
  }
}
...
for (const row of candidates) {
  ...
  const matchingOfficial = official.find((page) => {
    try { return new URL(page.finalUrl).pathname.toLowerCase() === sourcePath; } catch { return false; }
  });
  console.log(JSON.stringify({
    pathKey: row.pathKey,
    title: row.title,
    sourceUrl: row.sourceUrl,
    collectedAt: row.collectedAt,
    sourceMatches3DCelMatrix: sourcePath === "/3dcelmatrix.html",
    storedMarkers: markerMap(row.storedText),
    ...
    storedTextLength: row.storedText.length,
    currentOfficialTextLength: matchingOfficial?.textLength ?? null,
  }, null, 2));
}
