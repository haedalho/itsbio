...
function parseProduct(html,requested){
  ...
  const imageUrls=[];$(".woocommerce-product-gallery img,img.wp-post-image,.product .images img").each((_,img)=>{const u=norm(abs($(img).attr("data-large_image")||$(img).attr("data-src")||$(img).attr("src")||"",sourceUrl));if(/^https?:\/\//i.test(u)&&!imageUrls.includes(u))imageUrls.push(u)});const og=norm(abs($("meta[property='og:image']").attr("content")||"",sourceUrl));if(!imageUrls.length&&/^https?:\/\//i.test(og))imageUrls.push(og);
