...
function isPriceHeader(value) {
  return /^(price|cost|amount|your price|list price)$/i.test(cleanText(value));
}
...
function stripPriceColumnsFromTable($, table) {
  ...
  $table.find("tr").each((_, tr) => {
    ...
    [...remove].sort((a, b) => b - a).forEach((index) => cells.eq(index).remove());
  });
}

function cleanFragment(rawHtml, baseUrl) {
  if (!rawHtml) return "";
  ...
  root.find(".price,.product-price,.abm-price,[class*='price-box'],[class*='add-to-cart'],[class*='addtocart'],[class*='quantity']").remove();
...
function extractDocuments($, baseUrl, panels) {
  ...
  const add = (title, rawUrl, section = "") => {
    ...
    if (!/\.(pdf|docx?|xlsx?|zip)(?:[?#]|$)/i.test(url) && !/datasheet|msds|protocol|brochure|guide|document/i.test(`${title} ${url}`)) return;
    seen.add(url);
    docs.push({ title: cleanText(title) || "Document", url, section });
  };
...
function extractServiceDetailsHtml($, baseUrl) {
...
export function parseAbmRebuildDetail(html, sourceUrl, expected = {}) {
  ...
  const serviceDetailsHtml = kind === "service" ? extractServiceDetailsHtml($, sourceUrl) : "";
