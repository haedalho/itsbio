...
export function sanitizeAbmStoredHtml(rawHtml, baseUrl = ABM_BASE, options = {}) {
  if (!rawHtml) return "";
  const preserveLandingInteractions = options?.preserveLandingInteractions === true;
  const $ = cheerio.load(`<div id="__root">${rawHtml}</div>`, { decodeEntities: false });
  const root = $("#__root");

  root.find(`script,style,noscript,form,input,select,textarea${preserveLandingInteractions ? "" : ",button"}`).remove();
  ...
  root.find("td,th,span,div,p,strong").each((_, el) => {
    const node = $(el);
    const text = cleanText(node.text());
    // A plain "Quantity" table header describes the supplied product amount
    // (for example 1.0 ml) and is not a shopping-cart control. Cart quantity
    // widgets are removed above by their classes/containers.
    if (!node.children().length && (isPriceHeader(text) || isCurrencyValue(text) || /^add\s+to\s+cart$/i.test(text))) node.remove();
  });
  ...
  return sanitizeHtml(html, {
    allowedTags: sanitizeHtml.defaults.allowedTags.concat([
      ...
      ...(preserveLandingInteractions ? ["button", "svg", "path"] : []),
    ]),
    allowedAttributes: {
      a: ["href", "title", "target", "rel"],
      img: ["src", "alt", "title", "width", "height"],
      svg: preserveLandingInteractions
        ? ["viewBox", "viewbox", "width", "height", "fill", "stroke", "aria-hidden", "focusable"]
        : [],
      path: preserveLandingInteractions
        ? ["d", "fill", "stroke", "stroke-width", "stroke-linecap", "stroke-linejoin"]
        : [],
      td: ["colspan", "rowspan"],
      th: ["colspan", "rowspan", "scope"],
      button: preserveLandingInteractions
        ? ["type", "class", "aria-controls", "aria-expanded", "aria-pressed", "aria-label", "data-filter-group", "data-modal-image", "data-modal-title", "data-modal-description"]
        : [],
      "*": preserveLandingInteractions
        ? ["class", "id", "hidden", "role", "tabindex", "aria-label", "aria-hidden", "aria-modal", "aria-labelledby", "data-filter-group", "data-biology", "data-href", "data-link", "data-modal-image", "data-modal-title", "data-modal-description"]
        : ["class", "id"],
    },
    transformTags: {
